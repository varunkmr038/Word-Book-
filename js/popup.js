...
const popup = {
  search: async () => {
    ...
    try {
      const wordData = await fetch(queryURL).then((response) =>
        response.json()
      );
      ...
      source.setAttribute("src", `https://${wordData[0].phonetics[0].audio}`);
      ...
      for (let i = 0; i < wordData[0].meanings[0].definitions.length; i++) {
        if (
          typeof wordData[0].meanings[0].definitions[i].example != "undefined"
        ) {
